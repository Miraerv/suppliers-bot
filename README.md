...
| `/help` (личка) | Message | Справка для поставщика |
| `/help` (админ-группа) | Message | Админ-команды |
| `/suppliers` (админ-группа) | Message | Список поставщиков |
| `/bind <id> <компания>` (админ-группа) | Message | Создать/сменить привязку |
| `/unbind <id>` (админ-группа) | Message | Удалить привязку |
...
## Админ-команды (в группе `ADMIN_CHAT_ID`)

| Команда | Действие |
|---------|----------|
| `/suppliers` | Список всех поставщиков (id, username, статус, расписание) |
| `/bind <telegram_id> <компания>` | Создать или сменить привязку (сразу `approved`) |
| `/unbind <telegram_id>` | Удалить привязку |
| `/help` | Справка по админ-командам |

Примеры:

```
/bind 123456789 ООО Ромашка
/bind 123456789 7707083893
/unbind 123456789
```

В личке `/help` показывает справку для поставщика.
